import { useListaMedicos } from '../hooks/useListaMedicos';
...
export function MedicosPage() {
    ...
    const { data, isLoading, error } = useListaMedicos(page);
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500">Gerencie a equipe médica da clínica Voll.med</p>
                </div>
                <button
                    onClick={() => setShowForm(true)}
                    className="bg-voll-secondary hover:bg-voll-primary text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors shadow-sm"
                >
            ...
            {showForm && <MedicoForm onClose={() => setShowForm(false)} />}
                    ...
                    <>
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {data?.content.map((medico: Medico) => (
                                    <tr key={medico.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-gray-600 text-sm">{medico.email}</td>
                                        <td className="px-6 py-4">
